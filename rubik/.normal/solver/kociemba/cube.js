(function() {
  // Centers
  var B, BL, BR, KCube, D, DB, DBL, DF, DFR, DL, DLF, DR, DRB, F, FL, FR, L, R, U, UB, UBR, UF, UFL, UL, ULB, UR, URF, centerColor, centerFacelet, cornerColor, cornerFacelet, edgeColor, edgeFacelet;
  ...
  KCube = (function() {
    ...
    class KCube {
      ...
      clone() {
        return new KCube(this.toJSON());
      }

      // A class method returning a new random cube
      static random() {
        return new KCube().randomize();
      }
      ...
      move(arg) {
        ...
        for (k = 0, len = ref.length; k < len; k++) {
          ...
          for (x = l = 0, ref1 = power; (0 <= ref1 ? l <= ref1 : l >= ref1); x = 0 <= ref1 ? ++l : --l) {
            this.multiply(KCube.moves[face]);
          }
        }
        return this;
      }
    ...
    KCube.prototype.randomize = (function() {
    ...
    // x
    KCube.moves.push(new KCube().move("R M' L'").toJSON());

    // y
    KCube.moves.push(new KCube().move("U E' D'").toJSON());

    // z
    KCube.moves.push(new KCube().move("F S B'").toJSON());

    // u
    KCube.moves.push(new KCube().move("U E'").toJSON());

    // r
    KCube.moves.push(new KCube().move("R M'").toJSON());

    // f
    KCube.moves.push(new KCube().move("F S").toJSON());

    // d
    KCube.moves.push(new KCube().move("D E").toJSON());

    // l
    KCube.moves.push(new KCube().move("L M").toJSON());

    // b
    KCube.moves.push(new KCube().move("B S'").toJSON());
