...
// Compute the index b < (maxOur + 1)! for the permutation
  ...
  KCube.computeMoveTables = function(...tables) {
...
// single, double or inverse move
  ...
  pruningTableParams = {
    ...
    sliceTwist: [
      ...
      twist;
        [slice,
      twist] = current;
        newSlice = KCube.moveTables.FRtoBR[slice * 24][move] / 24 | 0;
        newTwist = KCube.moveTables.twist[twist][move];
    ...
    sliceFlip: [
      ...
      flip] = current;
        newSlice = KCube.moveTables.FRtoBR[slice * 24][move] / 24 | 0;
        newFlip = KCube.moveTables.flip[flip][move];
    ...
    sliceURFtoDLFParity: [
      ...
      slice,
      URFtoDLF] = current;
        newParity = KCube.moveTables.parity[parity][move];
        newSlice = KCube.moveTables.FRtoBR[slice][move];
        newURFtoDLF = KCube.moveTables.URFtoDLF[URFtoDLF][move];
    ...
    sliceURtoDFParity: [
      ...
      URtoDF] = current;
        newParity = KCube.moveTables.parity[parity][move];
        newSlice = KCube.moveTables.FRtoBR[slice][move];
        newURtoDF = KCube.moveTables.URtoDF[URtoDF][move];
        return (newURtoDF * N_SLICE2 + newSlice) * 2 + newParity;
      }
    ]
  };

  KCube.computePruningTables = function(...tables) {
  ...
  KCube.initSolver = function() {
    KCube.computeMoveTables();
    return KCube.computePruningTables();
  };

  KCube.prototype.solveUpright = function(maxDepth = 22) {
    ...
    State = class State {
      ...
      move(table, index, move) {
        return KCube.moveTables[table][index][move];
      }

      pruning(table, index) {
        return pruning(KCube.pruningTables[table], index);
      }
  ...
  KCube.scramble = function() {
    return KCube.inverse(KCube.random().solve());
  };
