...
class Cube {
    ...
    verify_cube() {
        ...
        const F = this.cubies["F"].colors()[0]
        const B = this.cubies["B"].colors()[0]
        const D = this.cubies["D"].colors()[0]
        const L = this.cubies["L"].colors()[0]
        const R = this.cubies["R"].colors()[0]
        const U = this.cubies["U"].colors()[0]
        ...
        let flip = (data) => Object.fromEntries(
            Object
              .entries(data)
              .map(([key, value]) => [value, key])
        )

        const corner_color = {"FLU": sortString(F + L + U),
            "FRU": sortString(F + R + U),
            "DFL": sortString(F + D + L),
            "DFR": sortString(F + D + R),
            "BLU": sortString(B + L + U),
            "BRU": sortString(B + R + U),
            "BDL": sortString(B + L + D),
            "BDR": sortString(B + R + D)
        }
        const color_corner = flip(corner_color)
        ...
        const edge_color = {"FU": sortString(F + U),
            "FL": sortString(F + L),
            "FR": sortString(F + R),
            "DF": sortString(D + F),
            "BU": sortString(B + U),
        ...
        const color_edge = flip(edge_color)
        ...
        function countSwaps(position, cubies, count, parity_count, isCorner) {
            ...
            const curr_color = cubies[position].colors().toSorted().join("")
            
            if (isCorner && curr_color != corner_color[position]) {
                return countSwaps(color_corner[curr_color], cubies, count + 1, parity_count, isCorner)
            }
            else if (!isCorner && curr_color != edge_color[position]) {
                return countSwaps(color_edge[curr_color], cubies, count + 1, parity_count, isCorner)
            }
            else {
